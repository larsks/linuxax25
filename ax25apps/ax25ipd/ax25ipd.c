...
char ptysymlink[PATH_MAX];   /* path to symlink to allocated pty device */
...
static struct option options[] = {
    {"version", 0, NULL, 'v'},     {"loglevel", 1, NULL, 'l'},
    {"help", 0, NULL, 'h'},        {"configfile", 1, NULL, 'c'},
    {"ttydevice", 1, NULL, 'd'},   {"nofork", 0, NULL, 'f'},
    {"symlink-pty", 1, NULL, 's'}, {NULL, 0, NULL, 0}};

static void greet_world(void) {
  printf("\nax25ipd %s\n", VERSION);
  printf("Copyright 1991, Michael Westerhof, Sun Microsystems, Inc.\n");
  printf("This software may be freely used, distributed, or modified, "
         "providing\nthis header is not removed\n\n");
  fflush(stdout);
}
...
int main(int argc, char **argv) {
  ...
  *ptysymlink = 0;
  ...
  if (opt_help == 1) {
    ...
    printf("  --symlink-pty PATH, -s PATH   Create symlink at PATH to "
           "allocated pty device\n");
